...
def _collect_mtp_rl_output(output_dict, model, postprocess_fn, *postprocess_args):
    """Retrieve MTP RL log_probs from the model, postprocess them, and attach to output_dict."""
    unwrapped = unwrap_model(model)
    if not hasattr(unwrapped, "_mtp_rl_output") or unwrapped._mtp_rl_output is None:
        return
    mtp_rl = unwrapped._mtp_rl_output
    for i in range(len(mtp_rl.mtp_log_probs)):
        mtp_rl.mtp_log_probs[i] = postprocess_fn(mtp_rl.mtp_log_probs[i], *postprocess_args, post_process=True)
    output_dict["mtp_rl_output"] = mtp_rl
    unwrapped._mtp_rl_output = None
    ...
    def model_forward(
        ...
        if data_format == "thd":
            ...
            if post_process and logits_processor is not None:
                ...
                _collect_mtp_rl_output(
                    output, model, postprocess_packed_seqs,
                    packed_seq_params, attention_mask, batch_size, seq_len,
                )
            else:
            ...
            if post_process and logits_processor is not None:
                ...
                _collect_mtp_rl_output(
                    output, model, postprocess_bshd,
                    new_attention_mask, attention_mask, sequence_length,
                )
            else:
