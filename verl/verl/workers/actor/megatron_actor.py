        ...
        if self.use_fused_kernels and not getattr(self.config, "overlap_moe_expert_parallel_comm", False):
            ...
            for model in self.actor_module:
                if self.mtp_config:
                    ...
                    if self.mtp_config.mtp_loss_type == "rl":
                        from verl.models.mcore.mtp_patch import patch_postprocess_rl

                        patch_postprocess_rl(model)
                    else:
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                mtp_rl_output = output.get("mtp_rl_output", None) if isinstance(output, dict) else None
                if mtp_rl_output is not None:
                    num_mtp_heads = len(mtp_rl_output.mtp_log_probs)
                    for k, mtp_log_prob in enumerate(mtp_rl_output.mtp_log_probs):
                        aligned_mtp_log_prob_resp = mtp_log_prob[:, -response_length - 1 : -1].contiguous()

                        with torch.no_grad():
                            scaling = self.mtp_config.mtp_loss_scaling_factor / num_mtp_heads
                            gradient_dot = (aligned_mtp_log_prob_resp - old_log_prob) * (log_prob - old_log_prob)
                            gradient_norm = (aligned_mtp_log_prob_resp - old_log_prob)**2
                            scaling = scaling * (gradient_dot / (gradient_norm+1e-8)).mean()


                        mtp_pg_loss_k, mtp_metrics_k = policy_loss_fn(
                            old_log_prob=old_log_prob,
                            log_prob=aligned_mtp_log_prob_resp,
                            advantages=advantages*scaling,
                            response_mask=response_mask,
                            loss_agg_mode=loss_agg_mode,
                            config=self.config,
                            rollout_is_weights=rollout_is_weights,
                        )
                        stats[f"mtp_losses/mtp_{k+1}_pg_loss"] = mtp_pg_loss_k.detach().item()
                        policy_loss = policy_loss + mtp_pg_loss_k
                        for mk, mv in mtp_metrics_k.items():
                            stats[f"mtp_losses/mtp_{k+1}_{mk}"] = mv
